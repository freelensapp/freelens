...
function calculatePodKubeMetrics(
  pod: Pod,
  metrics: any,
  ...
  return pod.getContainers().reduce((total, container) => {
    const metric = metrics.containers.find(
...
function getPodKubeMetricsMemoryDisplayValue(
  pod: Pod,
  metrics: any[],
): string {
...
const PodMetricsMemoryContent = observer(
  ({ pod, podMetrics }: { pod: Pod; podMetrics: { value: { get: () => any[] }}}) => {
    ...
    return (
      <span
        className="pod-metrics pod-memory"
        data-testid={`pod-memory-${pod.getId()}`}
      >
...
export const podsMetricsMemoryColumnInjectable = getInjectable({
  ...
  instantiate: (di) => {
    ...
    const podMetricsWatcherNamespaces = di.inject(
      podMetricsWatcherNamespacesInjectable,
    );
    ...
    return {
      ...
      content: (object: any) =>
        object instanceof Pod ? (
          <PodMetricsMemoryContent
            pod={object}
            podMetrics={podMetricsWatcher}
          />
        ) : null,
      header: {
        title: "Memory",
