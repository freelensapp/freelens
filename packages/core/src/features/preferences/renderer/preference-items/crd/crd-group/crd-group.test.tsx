...
vi.mock("@ogre-tools/injectable-react", () => ({
  withInjectables: vi.fn((component: any, config: any) => {
    return (props: any) => {
      // Create mock dependencies
      const mockDi = {
        inject: vi.fn(() => mockUserPreferencesState),
      };
...
describe("CrdGroup Component", () => {
  ...
  describe("Merge info counts", () => {
    it("reports zero user groups for empty config", () => {
      ...
      fireEvent.change(screen.getByTestId("monaco-editor-for-crd-group-config"), { target: { value: "" } });
