...
export const getMultiExportConfig = (packageJson: any, _dependencies: Partial<Dependencies>) => {
  ...
  if (invalidBuildTypes.length > 0) {
    throw new Error(
      `Tried to get multi export config for package "${packageJson.name}" but build types "${invalidBuildTypes.join(
        '", "',
...
const toExpectedExport = (externalImportPath: string) => {
  ...
  const entrypointPath = `./${posixJoinForPackageJson("./dist", externalImportPath, "index.js")}`;
...
const toExportSpecificWebpackConfigFor =
  ...
  ([externalImportPath, { buildType, entrypoint }]: [string, { buildType: string; entrypoint: string }]) => {
    const outputDirectory = dependencies.resolvePath!(dependencies.workingDirectory!, "dist", externalImportPath);
