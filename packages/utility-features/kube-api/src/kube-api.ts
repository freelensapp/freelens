  ...
  watch(opts?: KubeApiWatchOptions<Object, Data>): Disposer {
    ...
    responsePromise
      .then((response) => {
        ...
        // The body is a WHATWG `ReadableStream`, so the newline framing that a
        // kube watch response uses has to be reassembled here: one chunk can
        // carry several events, and one event can straddle two chunks.
        void (async () => {
