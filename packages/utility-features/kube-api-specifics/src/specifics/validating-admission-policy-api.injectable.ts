...
export const validatingAdmissionPolicyApiInjectable = getInjectable({
  id: "validating-admission-policy",
  instantiate: (di) => {
    assert(
      di.inject(storesAndApisCanBeCreatedInjectionToken),
      "const validatingAdmissionPolicyApi is only available in certain environments",
    );

    return new ValidatingAdmissionPolicyApi({
